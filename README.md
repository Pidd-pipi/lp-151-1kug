...
- 按帖树洞化名：每篇帖子为参与者独立派生一套匿名昵称与头像；同一身份在该帖的发帖与评论沿用同一套化名，换到另一篇帖子即更换，接口不返回任何可跨帖拼接的身份编号，历史数据升级后即时生效
...
- 匿名身份管理：可创建多个匿名身份，身份页展示原始昵称与头像，本地切换使用
...
| ALIAS_SECRET | 留空回退 JWT_SECRET | 按帖化名派生密钥，生产建议设置独立随机值并保持固定，更换会使全部历史帖子的化名整体变化 |

## 隐私设计：按帖树洞化名

- 列表、帖子详情与评论中展示的昵称/头像均为「该身份在本篇帖子内」的化名，由后端用 `ALIAS_SECRET` 对（帖子ID、身份ID）做 HMAC-SHA256 派生，无需也无法由客户端反推真实身份。
- 同一身份在同一篇帖子的发帖与评论始终得到同一组昵称与头像；换到另一篇帖子即得到完全不同的一组，读者无法通过昵称、头像或编号跨帖串联同一人的发言。
- 帖子与评论接口不再返回 `identityId` 等稳定身份标识；身份编号仅在登录后由后端内部使用。
- 身份页（`/identity`）仍展示当前身份的原始昵称、头像与身份密钥，便于本人管理。
- 化名为运行时派生，不依赖额外数据表，历史数据升级后自动适用；无需数据迁移。
